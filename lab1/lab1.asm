...
FILL_IDT:                                       ;Заполнить таблицу дескрипторов шлюзов прерываний
    ...
    irpc    N, 1234567                           ;Заполнить вектора 21-27 заглушками
...
DATA_BEGIN      = $
    ...
    ENDM
    SIZE_IDT        =       ($ - IDT_BEGIN)
    MSG_HELLO           db "Press any key to enter Protected Mode",13,10,"$"
    MSG_HELLO_PM        db "Entered Protected Mode",0
    MSG_EXIT            db "Back to Real Mode",13,10,"$"
    MSG_TIME            db "Go back to RM in  XXXXXXX seconds",0
    MSG_COUNT           db "Quantity of interrupt calls:",0
    MSG_EXC             db "Exception: XX",0
    MSG_ENTER           db "Enter time: $"
